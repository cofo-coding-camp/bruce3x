...
class Solution {
    fun findPoisonedDuration(timeSeries: IntArray?, duration: Int): Int {
        ...
        val seconds = BooleanArray(timeSeries.last() + duration + 1)

        for (i in 0 until timeSeries.size) {
            for (j in timeSeries[i] until timeSeries[i] + duration) {
                seconds[j] = true
            }
        }

        return seconds.count { it }
    }

    fun findPoisonedDuration2(timeSeries: IntArray?, duration: Int): Int {
